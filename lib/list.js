...
var {View, Text, StyleSheet, FlatList, Linking} = require("react-native");
...
function renderExportButton(values) {

  return (
    <Button
      style={{
        container: {
          width: 120, alignSelf: 'flex-end'
        }
      }}
      primary
      text="Export"
...
function exportCSV(values) {
  ...
  const header = keys.join('%2C') + '%0A'

  let rows = ''
  values.forEach((obj) => {
    const row = []
    keys.forEach((k, i) => row.push(obj[k]))
    rows += row.join('%2C') + '%0A'
  })
  
  Linking.openURL("data:text/csv;charset=utf-8,"+header+rows)
}
...
function list(locals) {
  ...
  if (locals.config.fields[locals.path[0]].addButton) locals.add.label = locals.config.fields[locals.path[0]].addButton
  ...
  return (
    <View style={fieldsetStyle}>
      ...
      />
      {renderExportButton(locals.value)}
      {addButton}
